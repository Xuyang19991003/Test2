...
def draw_one_figure(ax,x,method,sup,hard,soft,dataset):
    ...
    ax.set_ylim(0, 100)
    # 子标题
    ax.set_title(data2name[dataset],size=45,pad=20)
    # x轴坐标 和 y轴坐标
    ax.set_xlabel("监督训练数据的比例", size=40,fontname='SimSun',labelpad=15)
    #ax.set_xlabel("ratio of supervised data", size=30, labelpad=15)
    ax.set_ylabel("F1", size=40,labelpad=-15)

    # x轴和y轴刻度线的粗细（width） 和大小(labelsize)
    ax.tick_params(axis="x", width=2, labelsize=35)
    ax.tick_params(axis="y", width=2, labelsize=35)
...
def get_method_proference(method):
    figure, axes = plt.subplots(1, 4, sharex="none", sharey="none", figsize=(55, 13))
    ...
    for data in dataset_list:
        ...
        #图例
        if data=="Beer":
            ax.legend(loc='center left', bbox_to_anchor=(1, 0.5),ncol=1,prop={'size': 40})



    # 设置图例标题的字体

    plt.subplots_adjust(bottom=0.15,left=0.04, right=0.88)
